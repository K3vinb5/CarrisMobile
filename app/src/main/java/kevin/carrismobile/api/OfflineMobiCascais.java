...
public class OfflineMobiCascais {
    //keys constants
    public final static String ROUTES_KEY = "cascais_key_routes";
    public final static String STOP_TIMES_KEY = "cascais_key_stop_times";
    public final static String TRIPS_KEY = "cascais_key_trips";
    public final static String STOPS_KEY = "cascais_key_stops";

    public static void initMobiCascaisOffline(Activity activity){
        if (!Offline.mPrefsStops.contains(STOPS_KEY)){
            Offline.copyResource(R.raw.cascais_stops, STOPS_KEY,activity, Offline.mPrefsStops);
        ...
        if (!Offline.mPrefsStopTimes.contains(STOP_TIMES_KEY)){
            Offline.copyResource(R.raw.cascais_stop_times, STOP_TIMES_KEY, activity, Offline.mPrefsStopTimes);
        ...
        if (!Offline.mPrefsRoutes.contains(ROUTES_KEY)){
            Offline.copyResource(R.raw.cascais_routes, ROUTES_KEY, activity, Offline.mPrefsRoutes);
        ...
        if (!Offline.mPrefsTrips.contains(TRIPS_KEY)){
            Offline.copyResource(R.raw.cascais_trips, TRIPS_KEY, activity, Offline.mPrefsTrips);
        ...
        try{
            ...
            while (scanner.hasNextLine()) {
                ...
                if (nextLine[0].equals(id)){
                    ...
                    Direction direction = new Direction(nextLine[0], nextLine[3]);
        ...
        try{
            ...
            while(scanner.hasNextLine()){
                ...
                if (currentLine[0].equals(direction.getDirectionId())){
                    direction.addTrip(new Trip(currentLine[2], ""));
                }
            }
            scanner.close();
        }catch (Exception e){
